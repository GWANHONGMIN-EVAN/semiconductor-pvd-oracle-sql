...
| Data | Description |
|---|---|
| X_pvd_AlCu.csv | AlCu 공정 Sensor 데이터 |
| Y_pvd_AlCu.csv | AlCu 공정 Thickness 데이터 |
| X_pvd_WTi.csv | WTi 공정 Sensor 데이터 |
| Y_pvd_WTi.csv | WTi 공정 Thickness 데이터 |
...
```
      ...
      |                     |
      ↓                     ↓
SENSOR_MEASUREMENT  THICKNESS_MEASUREMENT
```

| Table | Description |
|---|---|
| PVD_RUN | 공정 Run 정보 및 소재 정보 관리 |
| SENSOR_MEASUREMENT | 증착 과정 Sensor 데이터 저장 |
| THICKNESS_MEASUREMENT | 증착 결과 Thickness 데이터 저장 |
...
- AlCu 공정은 WTi 대비 낮은 Thickness STD를 보여 상대적으로 균일한 증착 특성을 확인함.


### 5.2 Sensor Impact Analysis

Sensor 데이터와 Thickness 결과 간 상관관계를 분석하여 영향 가능성이 높은 Sensor를 탐색함.
...
- Sensor_40이 가장 높은 상관관계를 보임
- 특정 Sensor 단독 영향보다 여러 Sensor가 복합적으로 영향을 주는 형태로 판단함
...
- Sensor_88에서 높은 상관관계(0.72)를 확인함
- Thickness 변화와 관련성이 높은 주요 공정 변수 후보로 판단함
...
- Oracle Database 기반 관계형 데이터 모델 설계
